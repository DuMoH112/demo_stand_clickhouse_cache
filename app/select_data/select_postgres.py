import random

from Errors import error_db_handler
from Database.postgres import init_postgres
from select_data.utils import (
    timer,
    random_date,
    get_random_files
)


@error_db_handler
@init_postgres
@timer(isRetunrnTime=False)
def select_all_query_postgres(postgres_db, row=None):
    print(f"[INFO] SELECTION POSTGRES")
    files = select_files(postgres_db)
    if not files:
        return False

    functions = [
        (select_count_all_data, [postgres_db]),
        (select_sum_value_in_all_files, [postgres_db]),
        (select_min_and_max_dt, [postgres_db, get_random_files(files)]),
        (select_sum_value_in_one_files, [postgres_db, get_random_files(files)]),
        (select_with_filters, [postgres_db, get_random_files(files), 0]),
        (select_with_filters, [postgres_db, get_random_files(files), 1]),
        (select_with_filters, [postgres_db, get_random_files(files), 2]),
        (select_with_filters, [postgres_db, get_random_files(files), 3]),
        (select_with_filters, [postgres_db, get_random_files(files), 4]),
        (select_with_filters, [postgres_db, get_random_files(files), 5]),
        (select_with_filters, [postgres_db, get_random_files(files), 6]),
        (select_with_filters, [postgres_db, get_random_files(files), 7]),
        (select_with_filters, [postgres_db, get_random_files(files), 8]),
        (select_with_filters, [postgres_db, get_random_files(files), 9]),
        (select_with_filters, [postgres_db, get_random_files(files), 10])
    ]

    result = []
    repeat = 3
    all_cnt_query = repeat * (len(functions) - 1)
    for index, el in enumerate(functions):
        function, arguments = el
        # Прохождение по одному запросу несколько раз для получения среднего времени
        all_time = 0
        response = None
        # Пропускаем выбрку нексольких раз для первого запроса, так как он носит информативный характер
        if index > 0:
            for i in range(repeat):
                response = function(*arguments)
                iter_num = (i + 1) + ((index - 1) * repeat)
                if type(response) == dict:
                    all_time += response.get('total_time')
                    print(f"[+] Select {iter_num}/{all_cnt_query}. Time: {response.get('total_time')}")
            print(f"Avg time: {all_time / repeat}")
        else:
            response = function(*arguments)
                
        if type(response) == dict:
            result.append({
                "total_time": all_time / repeat,
                "result": response.get('result')
            })
        else:
            result.append(response)

    return result


@timer(isRetunrnTime=False)
def select_files(postgres_db):
    files = postgres_db.select_data("SELECT id FROM file")

    if files:
        files = [i[0] for i in files]

    return files


@timer(isRetunrnTime=False)
def select_count_all_data(postgres_db):
    count = postgres_db.select_data("SELECT count(*) FROM raw_data")[0]

    return count


@timer(isRetunrnTime=True)
def select_min_and_max_dt(postgres_db, files_id):
    list_files = ",".join([str(i) for i in files_id])
    response = postgres_db.select_data(f"""
        SELECT 
            min(dt),
            max(dt)
        FROM raw_data 
        WHERE file_id in ({list_files})
    """)

    return response


@timer(isRetunrnTime=True)
def select_sum_value_in_all_files(postgres_db):
    response = postgres_db.select_data("""
        SELECT 
            sum(value)
        FROM raw_data 
        GROUP BY file_id
    """)

    return response


@timer(isRetunrnTime=True)
def select_sum_value_in_one_files(postgres_db, files_id):
    list_files = ",".join([str(i) for i in files_id])
    response = postgres_db.select_data(f"""
        SELECT 
            sum(value)
        FROM raw_data
        WHERE file_id in ({list_files})
    """)

    return response


@timer(isRetunrnTime=True)
def select_with_filters(postgres_db, files_id, count_filters):
    other_filters = []
    if count_filters:
        for numb_filter in range(1, count_filters + 1):
            other_filters.append(
                f'filter_column{numb_filter}_id={random.randint(1, 4)}')

        other_filters = " and " + " and ".join(other_filters)
    else:
        other_filters = ""

    list_files = ",".join([str(i) for i in files_id])
    response = postgres_db.select_data(f"""
        SELECT
            sum(value)
        FROM raw_data
        WHERE
            file_id in ({list_files}) and
            dt='{random_date('2019-01-01', '2020-01-01', random.random())}'
            {other_filters}
        """)

    return response
